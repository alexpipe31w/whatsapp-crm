...
export class WhatsappController {
  ...
  async connect(@Param('storeId') storeId: string) {
    await this.whatsappService.connectStore(storeId);
    return { message: 'Conectando... escanea el QR en el terminal o usa GET /whatsapp/qr/:storeId' };
  }

  @Get('qr/:storeId')
  getQR(@Param('storeId') storeId: string, @Res() res: Response) {
  ...
  async disconnect(@Param('storeId') storeId: string) {
